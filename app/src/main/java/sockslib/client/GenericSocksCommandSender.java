...
public class GenericSocksCommandSender implements SocksCommandSender {
  ...
  public CommandReplyMessage checkServerReply(InputStream inputStream) throws SocksException,
    ...
    if (receivedData[3] == AddressType.IPV4) {
      ...
      //logger.debug("Server replied:Address as IPv4:{}.{}.{}.{}, port:{}", a, b, c, d, (UnsignedByte.toInt(portBytes[0]) << 8) | (UnsignedByte.toInt(portBytes[1])));
      ...
      //logger.debug("Server replied:Address as host:{}, port:{}", new String(addressBytes), (UnsignedByte.toInt(portBytes[0]) << 8) | (UnsignedByte.toInt(portBytes[1])));
      ...
      //Log.d("checkServerReply","Server replied:Address as IPv6:{}", new String(addressBytes));
    }
    ...
    Log.d("checkServerReply", "SOCKS server response success");
