  ...
  public void start(@Nullable String[] args) {
    ...
    try {
      ...
      if (useProxy) {
        ...
        Log.i("start", String.format("Connect server [%s:%d] by proxy:%s", host, port, proxy));
      ...
      Log.i("start", String.format("Server response:{}", receiveMessage));
      inputStream.close();
      outputStream.close();
      socket.close();
      Log.i("start", String.format("Total send:%d, Total receive: %d, Total: %d", monitor.getTotalSend(), monitor
