...
public class Socks5Handler implements SocksHandler {
  ...
  public void handle(Session session) throws Exception {
    ...
    Log.d("handle", String.format("SESSION[%d] Response client:%s", session.getId(), selectedMethod.getMethodName()));
    // send select method.
    ...
    if (commandMessage.hasSocksException()) {
      ...
      Log.i("handle", String.format("SESSION[%d] will close, because %s", session.getId(), serverReply));
  ...
  public void doConnect(Session session, CommandMessage commandMessage) throws SocksException,
    ...
    try {
      ...
      Log.i("doConnect", String.format("SESSION[%d] connect %s [%s] exception:{}", session.getId(), new
    ...
    while (pipe.isRunning()) {
      try {
        ...
        pipe.stop();
        session.close();
        Log.i("doConnect", String.format("SESSION[%d] closed", session.getId()));
      }
    }
  }
  ...
  public void doBind(Session session, CommandMessage commandMessage) throws SocksException,
    ...
    Log.i("doBind", String.format("Create TCP server bind at %s for session[%d]", serverSocket
    ...
    while (pipe.isRunning()) {
      try {
        ...
        pipe.stop();
        session.close();
        Log.i("doBind", String.format("Session[%d] closed", session.getId()));
      }
    }
    serverSocket.close();
