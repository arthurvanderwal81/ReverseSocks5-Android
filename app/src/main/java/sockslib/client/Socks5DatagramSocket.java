...
public class Socks5DatagramSocket extends DatagramSocket {
  ...
  public Socks5DatagramSocket(SocksProxy proxy) throws SocksException, IOException {
    ...
    Log.d("Socks5DatagramSocket", String.format("build datagram socket at [%s:%d]", this.getLocalAddress(), this.getLocalPort()));
    this.relayServerInetAddress = message.getIp();
    this.relayServerPort = message.getPort();

    Log.i("Socks5DatagramSocket", String.format("relay server's address[%s:%d]", relayServerInetAddress, relayServerPort));
  }
