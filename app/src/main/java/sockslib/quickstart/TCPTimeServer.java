...
public class TCPTimeServer implements Runnable {
  ...
  public void run() {
    ...
    try {
      ...
      Log.i("run", String.format("TCP time server created at %d", server.getInetAddress(), port));
      while (!stop) {
        ...
        Log.i("run", String.format("Client from %s send:%s", socket.getRemoteSocketAddress(), receive));
